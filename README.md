# 学校积分管理系统 (ScoreManagerForSchool)
...
> 基于 Avalonia 11 和 .NET 8 构建的现代化跨平台学校积分管理系统，提供完整的学生积分管理解决方案。

## 📋 核心功能

### 🎯 积分管理
- **快速录入**：支持批量录入和单个录入，默认2分积分设置
- **智能搜索**：基于拼音的中文姓名智能匹配
- **实时统计**：班级和个人积分实时统计分析
- **灵活筛选**：支持班级、时间范围、正负积分的多条件筛选

### 👥 学生管理
- **批量导入**：支持 CSV 格式学生名单导入
- **班级管理**：按班级分组管理学生信息
- **学号管理**：支持学号和姓名双重索引
- **快速查找**：支持拼音模糊搜索和精确匹配
...
### 📊 数据分析
- **统计报表**：班级积分统计、个人积分趋势
- **时间分析**：支持今日、昨日、本周、本月等快速时间选择
- **数据导出**：支持时间范围筛选的数据导出功能
- **可视化展示**：直观的数据图表和统计信息

### 🔒 安全保障
- **数据加密**：使用 PBKDF2 + AES-256-CBC 加密存储
- **设备绑定**：基于硬件特征的设备身份验证
- **访问控制**：密码保护和安全问题验证
- **自动备份**：重要数据自动备份机制
...
### 技术栈
- **前端框架**：Avalonia 11.0 (跨平台 UI)
- **后端框架**：.NET 8.0
- **数据存储**：CSV 文件 + 加密存储
- **中文处理**：NPinyin 0.2.6321 (拼音处理)
- **加密算法**：PBKDF2 + AES-256-CBC
